...
from database import ugovor_collection
...
async def dodaj_ugovor(ugovor: Ugovor, credentials: HTTPBasicCredentials = Depends(authenticate)):
    try:
        # Sprema ugovor u kolekciju Ugovor
        ugovor_collection.insert_one(ugovor.dict())
...
async def get_ugovori():
    try:
        ugovori = list(ugovor_collection.find({}, {'_id': 0}))  # Ne vraćamo '_id' polje
        return ugovori
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_ugovor(ugovor_id: str, ugovor: Ugovor):
    try:
        ...
        result = ugovor_collection.update_one({"_id": ObjectId(ugovor_id)}, {"$set": updated_ugovor})
...
async def delete_ugovor(ugovor_id: str):
    try:
        result = ugovor_collection.delete_one({"_id": ObjectId(ugovor_id)})
